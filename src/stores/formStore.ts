...
import { WHATSAPP_QUESTIONS, CALL_QUESTIONS } from '@/lib/constants'
import type {
  ChannelType,
  SalesStage,
  CheckAnswer,
...
export const useFormStore = create<FormStore>()((set, get) => ({
  ...
  initFromEvaluation: (ev) => {
    ...
    set({
      ...
      stageAnswers:       ev.stage_answers        ? (ev.stage_answers        as unknown as FormStageAnswers)       : { ...EMPTY_STAGE_ANSWERS },
      offerAnswers:       ev.offer_answers         ? (ev.offer_answers         as unknown as FormOfferAnswers)      : { ...EMPTY_OFFER_ANSWERS },
      dealAnswers:        ev.deal_answers          ? (ev.deal_answers          as unknown as FormDealAnswers)       : { ...EMPTY_DEAL_ANSWERS },
      secondVisitAnswers: ev.second_visit_answers  ? (ev.second_visit_answers  as unknown as FormSecondVisitAnswers): { ...EMPTY_SECOND_VISIT_ANSWERS },
      criteriaScores,
      channelChecks,
      criticalErrors,
