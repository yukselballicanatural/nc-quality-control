...
import { labelFor, scoreLevelLabel, textFor, translateEnglishToItalian } from '@/lib/localization'
import {
  CRITERIA,
  WHATSAPP_QUESTIONS,
  CALL_QUESTIONS,
...
import type { EvaluationWithRelations, Language } from '@/types'
...
function qText(lang: Language, item: { tr: string; en: string; it?: string }) {
  return localText(lang, item.tr, item.en, item.it)
}

function fmtResponseTime(v: unknown, lang: Language): { answer: string; score: number } {
  if (v === '<15') return { answer: '< 15 dak', score: 10 }
  if (v === '=15') return { answer: '= 15 dak', score: 5 }
  if (v === '>15') return { answer: localText(lang, '> 15 dak', '> 15 min', '> 15 min'), score: 0 }
...
function fmtYesNo(v: unknown, max: number, lang: Language): { answer: string; score: number } {
  if (v === 'yes') return { answer: localText(lang, 'Evet', 'Yes', 'Sì'), score: max }
  if (v === 'no')  return { answer: localText(lang, 'Hayır', 'No', 'No'),  score: 0 }
  return { answer: '—', score: 0 }
}

function fmtFollowUp(v: unknown, pts: number[], lang: Language): { answer: string; score: number } {
  const n = Number(v)
  const labels = lang === 'tr'
    ? ['Hiç takip yok', '1 kez takip', '2 kez takip', '3+ kez takip']
    : lang === 'it'
      ? ['Nessun follow-up', '1 follow-up', '2 follow-up', '3+ follow-up']
...
function fmtDelivery(v: unknown, lang: Language): { answer: string; score: number } {
  if (v === '<24h') return { answer: localText(lang, '< 24 saat', '< 24h', '< 24h'), score: 10 }
  if (v === '>24h') return { answer: localText(lang, '> 24 saat', '> 24h', '> 24h'), score: 0 }
  return { answer: '—', score: 0 }
}
...
function ModalBody({ ev }: { ev: EvaluationWithRelations }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-5 px-4 pb-8 sm:px-6">
      ...
      <div className={`rounded-2xl p-4 sm:p-5 flex items-center justify-between gap-4 ${level.bgColor}`}>
        <div>
          <p className={`text-xs font-bold uppercase tracking-wider opacity-60 mb-1 ${level.textColor}`}>
            {localText(lang, 'Final Puan', 'Final Score')}
          </p>
          ...
          <p className={`text-sm font-bold mt-1.5 opacity-75 ${level.textColor}`}>
            {scoreLevelLabel(lang, level)} · /100
          </p>
        </div>
        <div className="text-right">
          {stageMeta && (
            <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full bg-white/50 ${level.textColor} mb-2`}>
              ...
              {qText(lang, stageMeta)}
            </span>
          )}
          <div className={`text-xs font-medium opacity-60 ${level.textColor}`}>
            {localText(lang, 'Ham Puan', 'Raw Score')}: {ev.raw_score}
            {ev.critical_error_count > 0 && (
              <span className="ml-2 text-orange-600 font-bold">
                ⚠ {ev.critical_error_count} {localText(lang, 'kritik hata', 'critical error(s)')}
              </span>
            )}
            {ev.is_auto_failed && (
      ...
      {/* ── Temel bilgiler ─────────────────────────────────── */}
      <Section title={localText(lang, 'Temel Bilgiler', 'Basic Info')} icon={<User className="w-3.5 h-3.5" />}>
        <div className="bg-gray-50 rounded-2xl p-3.5 sm:p-4 grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
          ...
          {ev.team_leader_name && (
            <InfoCell label={localText(lang, 'Takım Lideri', 'Team Leader')} value={ev.team_leader_name} />
          )}
          {ev.region && (
            <InfoCell label={localText(lang, 'Bölge', 'Region', 'Regione')} value={ev.region} />
          )}
          ...
          {ev.evaluation_date && (
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-0.5">
                {localText(lang, 'Kontrol Tarihi', 'Control Date')}
              </p>
          ...
          {ev.team_leader && (
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-0.5">
                {localText(lang, 'Takım Lideri', 'Team Leader')}
              </p>
              <div className="flex items-center gap-1.5 font-medium text-gray-700">
          ...
          {ev.evaluator && (
            <div className="col-span-2">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-0.5">
                {localText(lang, 'Değerlendiren', 'Evaluator')}
              </p>
          ...
          {ev.general_note && (
            <div className="col-span-2">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-0.5">
                {localText(lang, 'Genel Not', 'General Note', 'Nota Generale')}
              </p>
      ...
      {isStageEval && (
        <>
          ...
          {(stageKey && !isDeal && !isSecondVisit) && (
            <Section title={localText(lang, 'Stage Soruları', 'Stage Questions')} icon={<ClipboardList className="w-3.5 h-3.5" />}>
              {sa ? (
                <div className="space-y-1.5">
                  <QuestionRow index={1}  label={qText(lang, STAGE_QUESTIONS.q1)}  {...fmtResponseTime(sa.q1ResponseTime, lang)}        maxScore={10} />
                  <QuestionRow index={2}  label={qText(lang, STAGE_QUESTIONS.q2)}  {...fmtScore4(sa.q2ProfessionalIntro)}               maxScore={10} />
                  <QuestionRow index={3}  label={qText(lang, STAGE_QUESTIONS.q3)}  {...fmtScore4(sa.q3NeedsAssessment)}                 maxScore={10} />
                  <QuestionRow index={4}  label={qText(lang, STAGE_QUESTIONS.q4)}  {...fmtScore4(sa.q4BuildTrust)}                      maxScore={10} />
                  <QuestionRow index={5}  label={qText(lang, STAGE_QUESTIONS.q5)}  {...fmtScore4(sa.q5InfoPhotoCollection)}             maxScore={10} />
                  <QuestionRow index={6}  label={qText(lang, STAGE_QUESTIONS.q6)}  {...fmtScore4(sa.q6ProfessionalComm)}                maxScore={10} />
                  <QuestionRow index={7}  label={qText(lang, STAGE_QUESTIONS.q7)}  {...fmtYesNo(sa.q7CrmDocumentation, 10, lang)}      maxScore={10} />
                  <QuestionRow index={8}  label={qText(lang, STAGE_QUESTIONS.q8)}  {...fmtYesNo(sa.q8ConfirmNextSteps, 10, lang)}       maxScore={10} />
                  <QuestionRow index={9}  label={qText(lang, STAGE_QUESTIONS.q9)}  {...fmtYesNo(sa.q9NoPriceDiscussion, 10, lang)}      maxScore={10} />
                  <QuestionRow index={10} label={qText(lang, STAGE_QUESTIONS.q10)} {...fmtFollowUp(sa.q10FollowUp, [0,1,2,3], lang)}   maxScore={3}  />
                </div>
              ) : (
                <NoAnswers lang={lang} />
              )}
          ...
          {isExtended && (
            <Section title={localText(lang, 'Teklif Soruları', 'Offer Questions')} icon={<ClipboardList className="w-3.5 h-3.5" />}>
              {oa ? (
                <div className="space-y-1.5">
                  <QuestionRow index={11} label={qText(lang, OFFER_QUESTIONS.q11)} {...fmtYesNo(oa.q11TreatmentPlanSent, 10, lang)}      maxScore={10} />
                  <QuestionRow index={12} label={qText(lang, OFFER_QUESTIONS.q12)} {...fmtDelivery(oa.q12DeliveryTiming, lang)}           maxScore={10} />
                  <QuestionRow index={13} label={qText(lang, OFFER_QUESTIONS.q13)} {...fmtYesNo(oa.q13ProfessionalPDF, 10, lang)}        maxScore={10} />
                  <QuestionRow index={14} label={qText(lang, OFFER_QUESTIONS.q14)} {...fmtYesNo(oa.q14ClearExplanation, 5, lang)}         maxScore={5}  />
                  <QuestionRow index={15} label={qText(lang, OFFER_QUESTIONS.q15)} {...fmtYesNo(oa.q15BeforeAfterPhotos, 5, lang)}        maxScore={5}  />
                  <QuestionRow index={16} label={qText(lang, OFFER_QUESTIONS.q16)} {...fmtYesNo(oa.q16ConciseCommunication, 5, lang)}     maxScore={5}  />
                  <QuestionRow index={17} label={qText(lang, OFFER_QUESTIONS.q17)} {...fmtFollowUp(oa.q17FollowUpWeek, [0,1,2,3], lang)}  maxScore={3}  />
                </div>
              ) : (
                <NoAnswers lang={lang} />
              )}
          ...
          {/* Deal soruları */}
          {isDeal && (
            <Section title={localText(lang, 'Deal Soruları', 'Deal Questions')} icon={<ClipboardList className="w-3.5 h-3.5" />}>
              {da ? (
                <div className="space-y-1.5">
                  <QuestionRow index={1} label={qText(lang, DEAL_QUESTIONS.q1)} {...fmtYesNo(da.q1CrmUpdated, 25, lang)}            maxScore={25} />
                  <QuestionRow index={2} label={qText(lang, DEAL_QUESTIONS.q2)} {...fmtYesNo(da.q2PriceConfirmed, 25, lang)}         maxScore={25} />
                  <QuestionRow index={3} label={qText(lang, DEAL_QUESTIONS.q3)} {...fmtYesNo(da.q3TravelConfirmed, 25, lang)}         maxScore={25} />
                  <QuestionRow index={4} label={qText(lang, DEAL_QUESTIONS.q4)} {...fmtFollowUp(da.q4FollowUp, [0,8,15,25], lang)}    maxScore={25} />
                </div>
              ) : (
                <NoAnswers lang={lang} />
              )}
          ...
          {/* Second Visit sorusu */}
          {isSecondVisit && (
            <Section title={localText(lang, 'İkinci Ziyaret Sorusu', 'Second Visit Question')} icon={<ClipboardList className="w-3.5 h-3.5" />}>
              {sva ? (
                <div className="space-y-1.5">
                  <QuestionRow
                    index={1}
                    label={qText(lang, SV_QUESTION)}
      ...
      {!isStageEval && (
        <Section title={localText(lang, 'Kriter Puanları', 'Criteria Scores')} icon={<TrendingUp className="w-3.5 h-3.5" />}>
          <div className="space-y-1.5">
            {CRITERIA.map(c => {
              const row = criteriaMap[c.number]
              const label = labelFor(lang, c)
              ...
              >
                <div className="space-y-1.5">
                  {questions.map(q => {
                    ...
                    const label = labelFor(lang, q)
                    const answerOpt = row ? CHECK_ANSWER_OPTIONS.find(o => o.value === row.answer) : null
                    const answerLabel = answerOpt ? labelFor(lang, answerOpt) : '—'
      ...
      {(ev.critical_errors ?? []).length > 0 && (
        <Section title={localText(lang, 'Kritik Hatalar', 'Critical Errors')} icon={<AlertTriangle className="w-3.5 h-3.5 text-red-400" />}>
          <div className="space-y-1.5">
      ...
      {/* ── Satış analizi ──────────────────────────────────── */}
      {hasSalesData && (
        <Section title={localText(lang, 'Satış Analizi', 'Sales Analysis')}>
          <div className="space-y-1.5">
            {[
              { label: localText(lang, 'Motivasyon anlaşıldı mı?', 'Motivation understood?'), val: ev.sales_understood_motivation },
              { label: localText(lang, 'Karar kolaylaştırıldı mı?', 'Decision eased?'), val: ev.sales_eased_decision },
              { label: localText(lang, 'Fırsat kullanıldı mı?', 'Opportunity used?'), val: ev.sales_opportunity_used },
              { label: localText(lang, 'Sonuç nedeni', 'Result reason'), val: ev.sales_result_reason },
              { label: localText(lang, 'En iyi davranış', 'Best behavior'), val: ev.sales_best_behavior },
              { label: localText(lang, 'Risk davranışı', 'Risk behavior'), val: ev.sales_risk_behavior },
      ...
      {/* ── Gelişim planı ──────────────────────────────────── */}
      {hasDevData && (
        <Section title={localText(lang, 'Gelişim Planı', 'Development Plan')} icon={<Calendar className="w-3.5 h-3.5" />}>
          <div className="space-y-1.5">
            {[
              { label: localText(lang, 'Güçlü Yönler', 'Strengths'), val: ev.dev_strengths },
              { label: localText(lang, 'Gelişim Alanları', 'Areas to Improve'), val: ev.dev_areas_to_improve },
              { label: localText(lang, 'Koçluk Konusu', 'Coaching Topic'), val: ev.dev_coaching_topic },
              { label: localText(lang, 'Ekip Lideri Yorumu', 'Team Leader Comment'), val: ev.dev_team_leader_comment },
              { label: localText(lang, 'Danışman Planı', 'Consultant Plan'), val: ev.dev_consultant_plan },
              { label: localText(lang, 'Tekrar Kontrol Tarihi', 'Recheck Date'), val: ev.dev_recheck_date },
            ].filter(f => f.val).map(f => (
              <div key={f.label} className="bg-white border border-gray-100 rounded-xl px-4 py-3">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">{f.label}</p>
                <p className="text-sm text-gray-700 leading-relaxed">{f.val}</p>
...
function NoAnswers({ lang }: { lang: Language }) {
  return (
    <div className="bg-gray-50 border border-dashed border-gray-200 rounded-xl px-4 py-3 text-xs text-gray-400 text-center">
      {localText(lang, 'Bu değerlendirme için cevaplar kaydedilmemiş.', 'Answers not recorded for this evaluation.')}
    </div>
  )
}
            ...
            <div className="flex-shrink-0 flex items-center justify-between px-4 py-3.5 sm:px-6 sm:py-4 border-b border-gray-100">
              <div className="min-w-0 flex-1 mr-3">
                <p className="text-sm sm:text-base font-bold text-gray-900 truncate">
                  {localText(lang, 'Değerlendirme Detayı', 'Evaluation Detail')}
                </p>
                {evaluation && (
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                {canEdit && evalId && (
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">{localText(lang, 'Düzenle', 'Edit')}</span>
                  </Link>
                )}
                <button
                  onClick={onClose}
              ...
              {loading && (
                <div className="flex flex-col items-center justify-center h-64 gap-3">
                  ...
                  <p className="text-sm text-gray-400">
                    {localText(lang, 'Yükleniyor...', 'Loading...', 'Caricamento...')}
                  </p>
                </div>
              )}
              {!loading && evaluation && (
